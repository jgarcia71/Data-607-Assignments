...
```{r}
ggplot(WorldPopulation_selct_tidy, aes(x=Country_Name, y=Population, colour = Year, group = Year)) + geom_line(linetype="solid", color="blue", size=1) + geom_point(shape = 22, size = 3, fill = "yellow") + ggtitle("World Population Change") + labs(x="Income", y="Population")
```
...
```{r}
ggplot(WorldPopulation_selct_count10_td, aes(x=Country_Name, y=Population, colour = Year, group = Year)) +  coord_flip() + geom_bar(stat="identity",position="dodge") + ggtitle("World Population Country (Top 10)") + labs(x="Country", y="Population")
